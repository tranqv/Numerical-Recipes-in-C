...
void nrc_gaujac ( sReal x[], sReal w[], int n, sReal alf, sReal bet )
{
   ...
   int i, its, j; 
   sReal alfbet, an, bn, r1, r2, r3; 
   dReal a, b, c, p1, p2, p3, pp, temp, z, z1; 
   ...
   for ( i = 1; i <= n; i ++ ) {
      ...
      if ( its > MAXIT ) nrc_error ( "too many iterations in nrc_gaujac" ); 
      x[i]= z; 
      w[i]= exp ( nrc_gammln ( alf + n )+ nrc_gammln ( bet + n )- nrc_gammln ( n + 1.0 )-
         nrc_gammln ( n + alfbet + 1.0 ) )*temp*pow ( 2.0, alfbet )/( pp*p2 ); 
   }
}
#undef EPS
#undef MAXIT

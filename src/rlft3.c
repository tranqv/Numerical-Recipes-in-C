...
void nrc_rlft3 ( sReal ***data, sReal **speq, unsigned long nn1, unsigned long nn2, 
   unsigned long nn3, int isign )
{
   ...
   dReal theta, wi, wpi, wpr, wr, wtemp; 
   sReal c1, c2, h1r, h1i, h2r, h2i; 

   if ( 1 +&data[nn1][nn2][nn3]-&data[1][1][1] != nn1*nn2*nn3 )
      nrc_error ( "nrc_rlft3: problem with dimensions or contiguity of data array\n" ); 
   c1 = 0.5; 
   c2 = - 0.5*isign; 
   ...
   nn[3]= nn3 >> 1; 
   if ( isign == 1 ) {
      nrc_fourn ( &data[1][1][1]- 1, nn, 3, isign ); 
      for ( i1 = 1; i1 <= nn1; i1 ++ )

   ...
   unsigned long *nlong, huffcode *hcode )
{
   ...
   hcode -> nch = nchin; 
   index = nrc_lvector ( 1, ( long )( 2*hcode -> nch - 1 ) ); 
   up =( long * ) nrc_lvector ( 1, ( long )( 2*hcode -> nch - 1 ) ); 
   nprob = nrc_lvector ( 1, ( long )( 2*hcode -> nch - 1 ) ); 
   for ( nused = 0, j = 1; j <= hcode -> nch; j ++ ) {
      nprob[j]= nfreq[j]; 
      hcode -> icod[j]= hcode -> ncod[j]= 0; 
      if ( nfreq[j] ) index[++ nused]= j; 
   }
   for ( j = nused; j >= 1; j -- ) nrc_hufapp ( index, nprob, nused, j ); 
   k = hcode -> nch; 
   while ( nused > 1 ) {
      node = index[1]; 
      index[1]= index[nused --]; 
      nrc_hufapp ( index, nprob, nused, 1 ); 
   ...
   *nlong = 0; 
   ...
   nrc_free_lvector ( nprob, 1, ( long )( 2*hcode -> nch - 1 ) ); 
   nrc_free_lvector ( ( unsigned long * ) up, 1, ( long )( 2*hcode -> nch - 1 ) ); 
   nrc_free_lvector ( index, 1, ( long )( 2*hcode -> nch - 1 ) ); 
}
#undef NRANSI

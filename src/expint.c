...
sReal nrc_expint ( int n, sReal x )
{
   ...
   int i, ii, nm1; 
   sReal a, b, c, d, del, fact, h, psi, ans; 

   nm1 = n - 1; 
   if ( n < 0 || x < 0.0 || ( x == 0.0 && ( n == 0 || n == 1 ) ) )
   nrc_error ( "bad arguments in nrc_expint" ); 
   else {
      if ( n == 0 ) ans = exp ( - x )/x; 
      else {
         if ( x == 0.0 ) ans = 1.0/nm1; 
            ...
            if ( x > 1.0 ) {
               ...
               nrc_error ( "continued fraction failed in nrc_expint" ); 
            } else {
               ...
               nrc_error ( "series failed in nrc_expint" ); 
            }
         }
      }
   }

...
void nrc_stifbs ( sReal y[], sReal dydx[], int nv, sReal *xx, sReal htry, sReal eps, 
   ...
   void ( *nrc_derivs )( sReal, sReal [], sReal [] ) )
{
   ...
   sReal *dfdx, **dfdy, *err, *yerr, *ysav, *yseq; 
   ...
   dfdx = nrc_vector ( 1, nv ); 
   dfdy = nrc_matrix ( 1, nv, 1, nv ); 
   ...
   for ( ; ; ) {
      for ( k = 1; k <= kmax; k ++ ) {
         xnew =( *xx )+ h; 
         if ( xnew == ( *xx ) ) nrc_error ( "step size underflow in nrc_stifbs" ); 
         nrc_simpr ( ysav, dydx, dfdx, dfdy, nv, *xx, h, nseq[k], yseq, nrc_derivs ); 
         xest = SQR ( h/nseq[k] ); 
   ...
   *hnext = h/scale; 
   ...
   nrc_free_matrix ( dfdy, 1, nv, 1, nv ); 
   nrc_free_vector ( dfdx, 1, nv ); 
